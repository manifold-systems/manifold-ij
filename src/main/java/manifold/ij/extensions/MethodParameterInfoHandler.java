...
//manifold: copy/pasted from JB to support optional params, this class is delegated to from ManMethodParameterInfoHandler
  ...
  DumbAware
{
  ...
  private static PsiExpressionList findMethodsForArgumentList(final CreateParameterInfoContext context,
    ...
    MethodParameterInfoHandler.CandidateWithPresentation[] items = ContainerUtil.map2Array(candidates, MethodParameterInfoHandler.CandidateWithPresentation.class,
      c -> new MethodParameterInfoHandler.CandidateWithPresentation(c,
        MethodParameterInfoHandler.MethodPresentation.from((PsiMethod)c.getElement(),
  ...
  public PsiExpressionList findElementForUpdatingParameterInfo(final @NotNull UpdateParameterInfoContext context) {
    ...
    if (expressionList != null) {
      ...
      if (candidates != null && candidates.length != 0) {
        ...
        MethodParameterInfoHandler.CandidateWithPresentation currentMethodInfo = null;
        if (highlighted instanceof PsiMethod method) {
          currentMethodInfo = (MethodParameterInfoHandler.CandidateWithPresentation)ContainerUtil.find(candidates, c -> getMethodFromCandidate(c).equals(method));
        } else if (highlighted instanceof MethodParameterInfoHandler.CandidateWithPresentation cwp) {
          currentMethodInfo = cwp;
        }
        if (currentMethodInfo == null) currentMethodInfo = (MethodParameterInfoHandler.CandidateWithPresentation)candidates[0];
  ...
  private static void updateParameterInfoInternal(@NotNull PsiExpressionList o, @NotNull UpdateParameterInfoContext context) {
    ...
    if( args.length == 1 && args[0] instanceof ManPsiTupleExpression )
    {
      ...
      args = tupleExpr.getExpressions();
    }

    int offset = context.getOffset();
    TextRange elRange = o.getTextRange();
    int index = getArgumentIndex( tupleExpr == null ? o : tupleExpr, offset, elRange );
    ...
    MethodParameterInfoHandler.CandidateWithPresentation chosenCandidate = null;
    MethodParameterInfoHandler.CandidateWithPresentation completeMatchCandidate = null;

    PsiExpression[] argsAppearanceOrder = args;

    for (int i = 0; i < candidates.length; i++) {
      MethodParameterInfoHandler.CandidateWithPresentation candidate = (MethodParameterInfoHandler.CandidateWithPresentation)candidates[i];
      CandidateInfo candInfo = candidate.candidateInfo;
      PsiMethod method = candidate.getMethod();
      if (!method.isValid()) continue;
      if (candInfo instanceof MethodCandidateInfo methodCandidateInfo && !methodCandidateInfo.getSiteSubstitutor().isValid()) continue;
      PsiSubstitutor substitutor = getCandidateInfoSubstitutor(candInfo, method == realResolve);
      ...
      if( tupleExpr != null )
      {
        index = getArgumentIndex( tupleExpr, offset, elRange );
        index = index < 0 || argsAppearanceOrder.length == 0 ? index : mapToParamIndex( argsAppearanceOrder[index], args );
        context.setCurrentParameter(index);
      }

      boolean enabled = true;
      if (parms.length <= index) {
        enabled = parms.length > 0
                  ? method.isVarArgs() && handleVarArgs( parms, substitutor, args, enabled, index )
                  : index == 0;
      }
      else {
      ...
      if (candidates.length > 1 && enabled) {
        ...
        if (realResolve == method) {
          if (parms.length == args.length && isAssignableParametersBeforeGivenIndex(parms, args, args.length, substitutor) ||
            ...
            completeMatchCandidate = candidate;
          }
        }
      }
    }

    MethodParameterInfoHandler.CandidateWithPresentation parameterToHighlight = chosenCandidate;
    if (completeMatchCandidate != null &&
      (chosenCandidate == null || (args.length > 0 && !(context.isPreservedOnHintHidden() &&
        Registry.is("editor.completion.hints.virtual.comma") &&
        completeMatchCandidate.presentation().parameters().size() <
          chosenCandidate.presentation().parameters().size())))) {
      parameterToHighlight = completeMatchCandidate;
    }
  ...
  private static int mapToParamIndex( PsiExpression arg, PsiExpression[] args )
  {
    return Arrays.asList( args ).indexOf( arg );
  }

  private static PsiExpression[] getTupleArgsInParamOrder( ManPsiTupleExpression tupleExpr, PsiExpression[] argsAppearanceOrder, PsiParameter[] params )
  {
    if( tupleExpr == null || !tupleExpr.isValid() )
    {
    ...
    {
      if( !nonlabeledArgs.isEmpty() )
      {
        args.add( nonlabeledArgs.remove( 0 ) );
      }
      else
      {
        PsiExpression labeledArg = labeledArgs.remove( param.getName() );
        args.add( labeledArg );
      }
    }

    return args.toArray( new PsiExpression[0] );
  }

  private static int getArgumentIndex( @NotNull PsiExpressionList o, int offset, TextRange elRange )
  {
    return offset <= elRange.getStartOffset() || offset >= elRange.getEndOffset()
      ? -1
      : ParameterInfoUtils.getCurrentParameterIndex( o.getNode(), offset, JavaTokenType.COMMA );
  }

  private static boolean handleVarArgs( PsiParameter[] parms, PsiSubstitutor substitutor, PsiExpression[] args, boolean enabled, int index )
  {
  ...
  private static void asyncHighlightHints(@NotNull Editor editor,
    ...
    Promise<MethodParameterInfoHandler.HighlightedInlays> inlaysPromise = Promises.resolvedPromise(new MethodParameterInfoHandler.HighlightedInlays(null, null));
  ...
  private static MethodParameterInfoHandler.HighlightedInlays collectInlaysToHighlight( Editor editor,
                                                                                                                                int currentHintIndex,
    ...
    if (expressionList == null || editor.isDisposed()) return new MethodParameterInfoHandler.HighlightedInlays(null, null);
    ...
    return new MethodParameterInfoHandler.HighlightedInlays(currentHint, highlightedHints);
  }
  ...
  private static PsiSubstitutor getCandidateInfoSubstitutor(CandidateInfo candidate, boolean resolveResult) {
    return candidate instanceof MethodCandidateInfo && ((MethodCandidateInfo)candidate).isInferencePossible()
      ? ((MethodCandidateInfo)candidate).inferTypeArguments(
        resolveResult ? DefaultParameterTypeInferencePolicy.INSTANCE : CompletionParameterTypeInferencePolicy.INSTANCE, true)
  ...
  private static boolean isAssignableParametersBeforeGivenIndex(final PsiParameter[] parms,
    ...
    for (int j = 0; j < length; j++) {
      ...
      if (!parmType.isAssignableFrom(argType) &&
        ...
        return RecursiveTypeVarEraser.eraseTypeVars( parmType, parm ).isAssignableFrom( RecursiveTypeVarEraser.eraseTypeVars( argType, arg ) );
      }
    }
    return true;
  }
  ...
  public static String updateMethodPresentation(@NotNull PsiMethod method, @Nullable PsiSubstitutor substitutor, @NotNull ParameterInfoUIContext context) {
    ...
    MethodParameterInfoHandler.MethodPresentation methodPresentation = MethodParameterInfoHandler.MethodPresentation.from(method, substitutor);
  ...
  public static String updateMethodPresentationFromCandidate(@NotNull ParameterInfoUIContext context, @NotNull Object candidate) {
    return updateMethodPresentation(context, ((MethodParameterInfoHandler.CandidateWithPresentation)candidate).presentation());
  }

  private static String updateMethodPresentation(@NotNull ParameterInfoUIContext context, @NotNull MethodParameterInfoHandler.MethodPresentation methodPresentation) {
    ...
    if (numParams > 0) {
      ...
      for (int j = 0; j < numParams; j++) {
        ...
        MethodParameterInfoHandler.ParameterPresentation parameterPresentation = methodPresentation.parameters().get(j);
        ...
        if( parameterPresentation.optional() )
        {
          buffer.append("[");
        }
  ...
  public void updateUI(final @NotNull Object p, final @NotNull ParameterInfoUIContext context) {
    updateMethodPresentation(context, ((MethodParameterInfoHandler.CandidateWithPresentation)p).presentation);
  }
  ...
  public static @NotNull PsiMethod getMethodFromCandidate(@NotNull Object object) {
    return ((MethodParameterInfoHandler.CandidateWithPresentation)object).getMethod();
  }

  public static @Nullable PsiMethod tryGetMethodFromCandidate(@NotNull Object object) {
    return object instanceof MethodParameterInfoHandler.CandidateWithPresentation cwp ? cwp.getMethod() : null;
  }

  private record CandidateWithPresentation(@NotNull CandidateInfo candidateInfo, @NotNull MethodParameterInfoHandler.MethodPresentation presentation) {
  ...
  private record ParameterPresentation(@NotNull String modifiers, @NotNull String type, @NotNull String name, @Nullable String javaDoc,
                                       boolean varArgs, boolean optional) {
    static @NotNull MethodParameterInfoHandler.ParameterPresentation from( @NotNull PsiParameter param, @NotNull PsiSubstitutor substitutor) {
      ...
      return new MethodParameterInfoHandler.ParameterPresentation(modifiers, type, name, javaDoc, param.isVarArgs(), hasInitializer( param ));
    }
  }
  ...
  private record MethodPresentation(@NotNull String modifiers, @NotNull String type, @NotNull String name, boolean constructor,
                                    ...
                                    @NotNull List<MethodParameterInfoHandler.ParameterPresentation> parameters) {
    static @NotNull MethodParameterInfoHandler.MethodPresentation from( @NotNull PsiMethod method, @NotNull PsiSubstitutor substitutor) {
