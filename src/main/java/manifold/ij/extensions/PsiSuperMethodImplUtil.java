...
// augment provider is reentered and cached Psi stuff is all wrong.  See also DelegationUtil.
//
public final class PsiSuperMethodImplUtil {
  private static final Logger LOG = Logger.getInstance( com.intellij.psi.impl.PsiSuperMethodImplUtil.class);
  private static final PsiCacheKey<Map<MethodSignature, HierarchicalMethodSignature>, PsiClass> SIGNATURES_FOR_CLASS_KEY = PsiCacheKey
    .create("MANIFOLD_SIGNATURES_FOR_CLASS_KEY",
  ...
  private static final PsiCacheKey<Map<Pair<String, GlobalSearchScope>, Map<MethodSignature, HierarchicalMethodSignature>>, PsiClass> SIGNATURES_BY_NAME_KEY = PsiCacheKey
    .create("MANIFOLD_SIGNATURES_BY_NAME_KEY", psiClass -> ConcurrentFactoryMap.createMap(
  ...
  private static Map<MethodSignature, HierarchicalMethodSignature> buildMethodHierarchy(@NotNull PsiClass aClass,
    ...
    Map<MethodSignature, HierarchicalMethodSignature> result = new com.intellij.util.containers.hash.LinkedHashMap<>(
    ...
    Map<MethodSignature, HierarchicalMethodSignatureImpl> map = new com.intellij.util.containers.hash.LinkedHashMap<>( new EqualityPolicy<MethodSignature>() {
      @Override
    ...
    PsiMethod[] methods = nameHint == null ? ((PsiExtensibleClass)aClass).getOwnMethods().toArray(new PsiMethod[0]) : aClass.findMethodsByName(nameHint, false);
